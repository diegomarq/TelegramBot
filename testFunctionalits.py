...
reply_keyboard = [['Receber notícias'], ['Outros']]
cli_opt_markup = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True, resize_keyboard=True)
...
#    bot.send_message(chat_id = client_data['chat_id'], text='Por favor digite seu login')
    
    update.message.reply_text('Olá ' + user.first_name + '!\nPara que eu possa te ajudar, preciso de mais algumas informações.')
    update.message.reply_text('Por favor digite seu login')
...
def get_login(bot, update, user_data):
#    client_data['login'] = str(args[0])
#    text = update.message.text
#    user_data['login'] = text

    text = update.message.text
    user_data['choice'] = text
    client_data['login'] = text    
    
    update.message.reply_text('Por favor digite sua senha')    
    return PWD_INFO
    
def get_pwd(bot, update, user_data):
#    client_data['pwd'] = str(args[0])
    
    text = update.message.text
    user_data['pwd'] = text
    client_data['pwd'] = text
    print(text)
    ...
    try:
        ...
        if(row is 'None'):
            ...
            """"""
            client_data['id_usuario_sistema'] = row.id_usuario_sistema
            client_data['log_user'] = row.NmoUsr
            """"""
            
            bot.send_message(chat_id = client_data['chat_id'], text = 'Encontrei sua conta!')
            bot.send_message(chat_id = client_data['chat_id'], text = 'Por favor, escolha uma opção:', reply_markup=cli_opt_markup)
...
#def cli_func_option(bot, update, user_data):
#    print('Entrou')
#    try:
#        text = update.message.text
#        user_data['choice'] = text
#        update.message.send_message('Opcao escolhida: ' + text, reply_markup=ReplyKeyboardRemove())
#    except Exception as e:
#        print(e)
...
def main():
    ...
    conv_handler = ConversationHandler(            
            ...
            states = {
                    ...
                    LOGIN_INFO: [MessageHandler(Filters.text, get_login)],
                    
                    PWD_INFO: [MessageHandler(Filters.text, get_pwd)]
...
#                   CLIENT_OPTION: [RegexHandler('^(Receber noticias)$', cli_func_option, pass_user_data=True),
#                                  RegexHandler('^(Outros)$', cli_func_option, pass_user_data=True)]
            },     
...
#    dp.add_handler(CallbackQueryHandler(cli_func_action))
